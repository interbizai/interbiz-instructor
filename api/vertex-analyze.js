...
# 응답 JSON 스키마 (반드시 이 구조로만 응답)
{
  ...
  // ⚠ 반복어 엄격 검증 규칙(habits):
  //  1) 진짜 '의미 없이' 반복되는 군더더기만 포함. 제품명/고유명사/필수 전문용어/조사('은/는','이/가','을/를'), 접속사('그리고','그래서') 같은 정상 단어 제외
  //  2) 같은 표현이 강의 전체에서 3회 이상 들렸을 때만 habits에 등록
  //  3) 부분 일치 아닌 정확 일치(예: "같은 경우는"의 '같은'은 조사구 일부이므로 단독 등록 금지)
  //  4) count는 occurrences 길이와 반드시 일치. occurrences의 context는 실제 영상에서 들린 문장의 원문 인용
  //  5) 반복어로 의심되지만 문맥상 필요한 단어이면 habits에서 제외
  "engagement_gaps_minutes": [{"timestamp":"MM:SS (환기가 필요한 시점 또는 지루함이 감지된 순간)","gap_minutes":int (직전 환기부터 이 시점까지의 분),"observation":"이 시점에서 관찰된 상황 (예: 수강생 반응 저조, 일방 설명이 10분 지속 등)","suggestion":"이 지점에 넣으면 좋을 구체적 환기 액션 (예: '제품 직접 만져보게 하기', '1분 짝토론' 등)"}],
